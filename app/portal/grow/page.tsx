...
export default function GrowPortal() {
  const [form, setForm] = useState({
    repName: "", accountName: "", city: "", visitDate: "",
    isNewAccount: false, sku: "", qtyOrdered: "", unitWholesale: "", notes: ""
  });
  ...
  const submit = async () => {
    if (!form.repName || !form.accountName || !form.visitDate) {
      alert("Please fill in Rep Name, Account Name, and Visit Date."); return;
    }
    setSubmitting(true);
    try {
      const res = await fetch("/api/grow/visit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, qtyOrdered: Number(form.qtyOrdered), unitWholesale: Number(form.unitWholesale) })
      });
      const data = await res.json();
      if (res.ok) {
        setMessage("✅ Visit submitted. HubSpot deal created.");
        setForm({ repName: "", accountName: "", city: "", visitDate: "", isNewAccount: false, sku: "", qtyOrdered: "", unitWholesale: "", notes: "" });
      } else {
    ...
    <div className="pw">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pw { --ink: #0a0a0a; --bone: #faf0ea; --gold: #d97706; --gold-pale: #fef3c7; --white: #fff; --muted: #767672;
          --shadow: 4px 4px 0 0 #0a0a0a; font-family: 'Manrope', sans-serif; min-height: 100vh;
          background: var(--bone); color: var(--ink); -webkit-font-smoothing: antialiased; }
        .pw-header { background: var(--ink); padding: 20px 32px; display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid var(--ink); }
        ...
        .pw-input::placeholder { color: rgba(10,10,10,0.3); }
        ...
        .pw-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: var(--shadow); }
      ...
      <div className="pw-header">
        <div>
          <p className="pw-logo">Greenline Activations — Team Portal</p>
          <p className="pw-title">GROW · Post-Visit Sales Recap</p>
        </div>
        <UserButton />
      </div>
      ...
      <div className="pw-body">
        <div className="pw-card">
          ...
          <div className="pw-grid">
            <div>
              <label className="pw-label">Sales Rep Name *</label>
              <input className="pw-input" name="repName" value={form.repName} onChange={handleChange} placeholder="e.g. Jordan Smith" />
            </div>
            <div>
              <label className="pw-label">Visit Date *</label>
              <input className="pw-input" type="date" name="visitDate" value={form.visitDate} onChange={handleChange} />
            </div>
            <div>
              <label className="pw-label">Account / Store Name *</label>
              <input className="pw-input" name="accountName" value={form.accountName} onChange={handleChange} placeholder="e.g. Total Wine & More" />
            </div>
            <div>
              <label className="pw-label">City / Market</label>
              <input className="pw-input" name="city" value={form.city} onChange={handleChange} placeholder="e.g. Orlando, FL" />
            </div>
            <div className="pw-full">
              <label className="pw-label">Account Status</label>
              <div className={`pw-checkbox-row ${form.isNewAccount ? "checked" : ""}`} onClick={() => setForm(prev => ({ ...prev, isNewAccount: !prev.isNewAccount }))}>
                <input className="pw-checkbox" type="checkbox" name="isNewAccount" checked={form.isNewAccount} onChange={handleChange} />
                <span className="pw-checkbox-label">New Account — first order with this location</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pw-card">
          ...
          <div className="pw-grid">
            <div>
              <label className="pw-label">SKU / Product</label>
              <input className="pw-input" name="sku" value={form.sku} onChange={handleChange} placeholder="e.g. GRW-GUMMY-100MG" />
            </div>
            <div>
              <label className="pw-label">Cases Ordered (Qty)</label>
              <input className="pw-input" type="number" min="0" name="qtyOrdered" value={form.qtyOrdered} onChange={handleChange} placeholder="0" />
            </div>
            <div>
              <label className="pw-label">Unit Wholesale Price ($ / case)</label>
              <input className="pw-input" type="number" min="0" step="0.01" name="unitWholesale" value={form.unitWholesale} onChange={handleChange} placeholder="0.00" />
            </div>
            <div style={{display: "flex", alignItems: "flex-end"}}>
              <div className="pw-total" style={{width: "100%"}}>
                <span className="pw-total-label">Line Total</span>
                <span className="pw-total-value">${lineTotal.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pw-card">
          ...
          <div>
            <label className="pw-label">Notes / Recap</label>
            <textarea className="pw-input pw-textarea" name="notes" value={form.notes} onChange={handleChange} placeholder="Buyer feedback, shelf placement, competitor observations, follow-up actions..." />
          </div>
          <button className="pw-btn" onClick={submit} disabled={submitting}>
            {submitting ? "Submitting..." : "Submit Visit Recap"}
          </button>
