...
const TARGET_BRAND = "GROOVEWAGON";
...
export default function UnifiedDashboard() {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        const res = await fetch('/api/dropdown-data?client=GROOVEWAGON');
        if (res.ok) {
          const data = await res.json();
  ...
  const fetchPhotos = async (force = false) => {
    ...
    try {
      const res = await fetch('/api/photos?client=groovewagon');
      const data = await res.json();
    ...
    <div className="dashboard-wrapper">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .dashboard-wrapper {
          ...
          --canopy: #0d9488; --canopy-pale: #ccfbf1;
      ...
      <div className="sidebar">
        ...
        <p className="sidebar-brand">GROOVEWAGON</p>
