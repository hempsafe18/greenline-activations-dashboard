...
const CLIENT_ID = "FOREST_GOLD_WATER";

export default function ForestGoldWaterDashboard() {
  const { user } = useUser();
  ...
  const fetchStats = async () => {
    setLoading(true);
    try { const res = await fetch("/api/forest-gold-water/stats"); if (res.ok) setStats(await res.json()); } catch {}
    setLoading(false);
  };
  ...
  const fetchUpcomingMeetings = async () => {
    try {
      const res = await fetch('/api/forest-gold-water/upcoming-meetings');
  ...
  const refreshAllRecaps = async () => {
    ...
    setRefreshMessage('');
    try {
      const res = await fetch('/api/forest-gold-water/revalidate', { method: 'POST', headers: { 'Authorization': 'Bearer dev-secret', 'Content-Type': 'application/json' } });
      if (res.ok) {
      ...
      <div className="db-layout">
        <aside className="db-sidebar">
          <div className="db-sidebar-header">
            ...
            <p className="db-sidebar-title">FOREST GOLD WATER</p>
            <p className="db-sidebar-sub">OnDeck Sales · Water Guru</p>
          </div>
        ...
        <main className="db-main">
          <div className="db-topbar">
            <div>
              ...
              <p className="db-topbar-meta">Welcome back, {user?.firstName||user?.emailAddresses?.[0]?.emailAddress?.split("@")[0]||"User"} &nbsp;·&nbsp; Forest Gold Water · Water Guru Activations · FL · Live Data</p>
            </div>
